...
class SocketManager {
    ...
    this.io.on("connection", (socket) => {
      ...
      socket.on("authenticate", async (data) => {
        try {
          ...
          console.log(`User authenticated: ${user.username} (${clerkUserId})`);
          socket.emit("authenticated", { success: true, username: user.username });
        } catch (error) {
      ...
      socket.on("join-room", async (data) => {
        try {
          ...
          let participant = room.participants.find(p => p.userId._id.toString() === userId);
          if (!participant) {
          ...
          socket.to(roomCode).emit("user-joined", {
            user: room.participants.find(p => p.userId._id.toString() === userId),
          });
      ...
      // Manual start duel (host only)
      socket.on("start-duel", async () => {
        try {
          const roomCode = socket.roomCode;
          const userId = socket.userId;
          ...
          if (room.host.toString() !== userId) {
            socket.emit("error", { message: "Only host can start the duel" });
            return;
          }
      ...
      socket.on("submit-code", async (data) => {
        try {
          ...
          const isCorrect = await this.executeAndVerifyCode(code, language, room.problem);
  ...
  async checkDuelEnd(roomCode) {
    try {
      const room = await Room.findOne({ roomCode }).populate("participants.userId");
      if (!room) return;

      const correctSubmissions = room.participants.filter(p => p.isCorrect && p.hasSubmitted);
      const allSubmitted = room.participants.every(p => p.hasSubmitted);

      // If someone got it right first, they win
      if (correctSubmissions.length > 0) {
        const winner = correctSubmissions.sort((a, b) => 
  ...
  async endDuel(roomCode, winnerId) {
    try {
      ...
      if (winnerId) {
        ...
        const losers = room.participants.filter(p => p.userId._id.toString() !== winnerId.toString());
