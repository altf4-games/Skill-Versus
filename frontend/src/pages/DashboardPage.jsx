...
export function DashboardPage() {
  ...
  const { user, syncUser, setNeedsProfileSetup } = useUserContext()
    ...
    <div className="space-y-8">
      ...
      {process.env.NODE_ENV === 'development' && (
        <Card className="border-dashed border-orange-300 bg-orange-50/50">
          <CardHeader>
            <CardTitle className="text-orange-700">Debug Tools</CardTitle>
            <CardDescription className="text-orange-600">
              Development tools for testing profile setup flow
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="text-sm text-orange-700">
              ...
              <pre className="bg-orange-100 p-2 rounded text-xs overflow-auto">
                {JSON.stringify({
                  ...
                  hasAutoGeneratedUsername: user?.username?.startsWith('user_')
                }, null, 2)}
              </pre>
            </div>
            <Button
              ...
              size="sm"
              onClick={() => {
                console.log('Manually triggering profile setup')
